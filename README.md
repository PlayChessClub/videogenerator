...
- **试试手气 / 试试手气 Pro**：视频 / 图片 / 语音三页均有。「试试手气」从本地词库随机取一条（不联网、不计费）；「试试手气 Pro」（金色按钮）调用 `qwen3.7-text-embedding-flash`，把「灵感种子」与候选词一起向量化，按余弦相似度挑出语义最贴合的提示词——与其他生成一样：先弹预计消耗确认，再调用并按实际 token 计入账单。
...
| 文本向量 | `qwen3.7-text-embedding-flash`（「试试手气 Pro」语义匹配，¥0.000125/千 token） |
